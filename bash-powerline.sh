...
__powerline() {
    ...
    ps1() {
        ...
        if [[ -n $VIRTUAL_ENV ]]; then
            local venv_path="$COLOR_VENV($(basename $VIRTUAL_ENV)) $COLOR_RESET"
        else
            local venv_path=""
        fi
        ...
        local user="$COLOR_GIT\u$COLOR_RESET"
        local host="$COLOR_GIT\h$COLOR_RESET"
        local monkey="$COLOR_GIT@$COLOR_RESET"

        PS1="$venv_path$user$monkey$host $cwd$git$symbol"
    }
